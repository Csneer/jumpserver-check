...
<div class="shell app-locked" id="appShell">
  <section class="hero">
    ...
    <div class="control-card">
      ...
      <div class="form-grid">
        ...
        <div class="field"><label for="stateFilter">状态筛选</label><select id="stateFilter"><option value="">全部候选</option><option value="missing_confirmation" selected>待确认</option><option value="confirmed">已确认</option><option value="confirmed_wait_next_scheduled_run">等待下次巡检</option><option value="stale_confirmation">确认已过期</option><option value="ip_reachable_requires_review">IP可达需复核</option><option value="tcp_open_requires_review">SSH端口开放需复核</option></select></div>
  ...
  <section class="panel">
    ...
    <div class="table-wrap"><table><thead><tr><th>资产</th><th>IP / 节点</th><th>操作</th><th>确认状态</th><th>失败原因</th><th>IP 可达性</th><th>Ping 时间</th><th>TCP 可达性</th><th>TCP 时间</th><th>证据 run</th></tr></thead><tbody id="rows"></tbody></table></div>
    <div id="cards"></div>
...
<script>
let latestData={candidates:[],review_required:[],skipped:[],summary:{},profiles:[]};
let currentProfile='';
...
const STATE_LABELS={missing_confirmation:'待确认',confirmed:'已确认',confirmed_wait_next_scheduled_run:'等待下次巡检',stale_confirmation:'确认已过期',invalid_confirmation:'确认无效',delete:'待删除',ip_reachable_requires_review:'IP可达需复核',tcp_open_requires_review:'SSH端口开放需复核'};
...
function allRows(){const candidates=(latestData.candidates||[]).map(c=>({...c,__kind:'candidate'}));const reviews=(latestData.review_required||[]).map(c=>({...c,confirmation_state:c.reason||'ip_reachable_requires_review',latest_reason:c.ip_reachability_remark||c.tcp_reachability_remark||c.latest_reason||'',__kind:'review'}));return candidates.concat(reviews);} function filtered(){const q=document.getElementById('search').value.trim().toLowerCase();const sf=document.getElementById('stateFilter').value;return allRows().filter(c=>{const text=[c.asset_name,c.asset_id,c.asset_ip,c.node,c.latest_reason,c.confirmation_state,c.confirmation_reason,c.ip_reachability,c.ip_reachability_checked_at,c.ip_reachability_remark,c.tcp_reachability,c.tcp_reachability_checked_at,c.tcp_reachability_remark].join(' ').toLowerCase();return (!q||text.includes(q))&&(!sf||c.confirmation_state===sf);});}
...
function tcpEvidence(c){return c.tcp_reachability?`${esc(c.tcp_reachability)}<br><span class="muted">${esc(c.tcp_reachability_remark||'-')}</span>`:'-';}
